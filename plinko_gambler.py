...
balls_in_play = 0
streak_counter = 0
jackpot = 100  # Starting jackpot value
...
def collision_handler(arbiter, space, data):
    ...
    # Apply small bonus after 3 consecutive wins
    if streak_counter >= 3 and score_change > 0:
        bonus = LEVELS[current_level]["ball_cost"] * 0.5  # 50% of ball cost as bonus
        current_money += bonus
        profit += bonus
        print(f"Streak bonus! ${bonus:.2f} added to your money!")

    # Jackpot logic
    if bucket_index == 4:  # Center bucket
        current_money += jackpot
        profit += jackpot
        print(f"Jackpot won! ${jackpot} added to your money!")
        jackpot = 100  # Reset jackpot
    else:
        jackpot += int(LEVELS[current_level]["ball_cost"] * 0.1)  # Increase jackpot
